...
from pollock.models.model import PollockModel, fit_model
from pollock.utils.dataloaders import get_train_dataloaders, get_prediction_dataloader
...
def convert_rds(rds_fp):
    h5_fp = rds_fp[:-4] + 'h5Seurat'
    h5ad_fp = rds_fp[:-4] + 'h5ad'
    subprocess.check_output(('Rscript', CONVERT_RDS_SCRIPT, rds_fp, h5_fp))
    return h5_fp, h5ad_fp
